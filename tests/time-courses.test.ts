...
test('course metadata survives D1, HTTP rollover, backup restore and repeated HTTP calls without version growth',async()=>{
 ...
 const request=()=>new Request('https://test/api/time/courses/rollover',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({space:'personal'})});const first=await (await handleTimeRequest(request(),store)).json() as any;assert.equal(first.changed,true);assert.equal(first.snapshot.plans.length,1);const second=await (await handleTimeRequest(request(),store)).json() as any;assert.equal(second.changed,false);assert.equal(second.version,first.version);assert.deepEqual(second,{version:first.version,changed:false});assert.deepEqual(await store.snapshot('personal','summary'),first.snapshot);
