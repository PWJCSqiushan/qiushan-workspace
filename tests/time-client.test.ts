...
test('rapid repeated undo while offline keeps one queued operation and replays once',async()=>{
 const original=globalThis.fetch;let offline=false,version=1,calls=0;const snapshot=()=>({owner:'undo-double-click',version,categories:[],intervals:[],plans:[],timer:null,imports:[],corrections:[]});
 globalThis.fetch=async(url)=>{if(offline)throw new TypeError('offline');if(String(url).includes('/api/session'))return Response.json({owner:'undo-double-click',expiresAt:Date.now()+60000});if(String(url).includes('/mutations')){calls++;version++;return Response.json({version,snapshot:snapshot()});}if(String(url).includes('/rollover'))return Response.json({version,changed:false});return Response.json(snapshot());};
 try{const c=new TimeClient('personal',()=>{});await c.start();offline=true;await Promise.all([c.enqueue({type:'undo',correctionId:'c1'}),c.enqueue({type:'undo',correctionId:'c1'})]);assert.equal(c.pending.length,1);offline=false;await c.sync();assert.equal(c.pending.length,0);assert.equal(calls,1);}finally{globalThis.fetch=original;}
});
