...
export type Receipt = {tasks?:Task[];queueVersion?:number;task?:Task;settings?:Workspace['settings'];settingsVersion?:number;cursor:number;operationId:string;serverTime:string};
...
export function applyTaskMutation(old:Task|null,m:Mutation,now=new Date()):Task {
 ...
 if(op.kind==='create'){
  ...
  return validateTask({...op.task,queueReturn:undefined,version:1,createdAt:stamp,updatedAt:stamp,completions:[],deletedAt:null,boostDate:'',boostAt:''});
 }
