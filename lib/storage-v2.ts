...
export class Store {
 ...
 async snapshot(space:Space):Promise<{workspace:Workspace;settingsVersion:number}>{const rows=await this.db.batch([
  ...
  return {workspace:{schemaVersion:1,space,revision:head.revision,tasks:(rows[1].results as Row[]).map(r=>({...JSON.parse(r.payload),version:r.version,completions:completions.get(r.task_id)||[]})),settings:JSON.parse(setting.payload)},settingsVersion:setting.version};}
 async sync(space:Space,cursor:number|null):Promise<SyncResult>{
  ...
  const changes=logs.slice(0,100).map(x=>({seq:x.seq,kind:x.kind,dto:JSON.parse(x.dto_json) as Omit<Receipt,'cursor'>}));return {mode:'delta',space,changes,nextCursor:changes.at(-1)?.seq??cursor,hasMore:logs.length>100,settingsVersion:(r[2].results[0] as Row).version,serverTime:new Date().toISOString()};
 }
 ...
 async mutate(input:unknown):Promise<Receipt>{
  ...
  try{await this.db.batch(statements);}catch(e){const retry=await this.receipt(m.space,m.operationId,hash);if(retry)return retry;if(next){const relationError=await this.relationError(m.space,next);if(relationError)throw new AppError(relationError,409);const latest=await this.task(m.space,m.taskId!);if(m.operation.kind==='create'?!!latest:latest?.version!==m.baseVersion)throw new ConflictError(latest);}else if((await this.snapshot(m.space)).settingsVersion!==m.baseVersion)throw new AppError('显示设置已发生变化',409);throw e;}
  return (await this.receipt(m.space,m.operationId,hash))!;
 }
 async preview(space:Space,input:unknown,kind:'migration'|'restore'){
  const w=validateImport(input);if(w.space!==space)throw new AppError('备份所属工作区不一致');if(!Number.isSafeInteger(w.revision)||w.revision<0)throw new AppError('源版本无效');
  ...
  const sourceHash=await sha256(w),current=await this.snapshot(space);return {sourceHash,taskCount:w.tasks.length,completionCount:w.tasks.reduce((n,t)=>n+t.completions.length,0),expectedRevision:current.workspace.revision,sourceRevision:w.revision,kind};
 }
 async importWorkspace(space:Space,input:unknown,runId:string,sourceHash:string,expectedRevision:number,kind:'migration'|'restore'){
  ...
  const beforeSnapshot=await this.snapshot(space);const before=beforeSnapshot.workspace;if(before.revision!==expectedRevision)throw new ConflictError(null);if(kind==='migration'&&(before.tasks.length||before.revision))throw new AppError('初次迁移要求空目标库',409);
  const stamp=new Date().toISOString(),revision=kind==='migration'?w.revision:before.revision+1;
  const tasks=kind==='migration'?w.tasks:w.tasks.map(t=>({...t,version:Math.max(t.version,before.tasks.find(x=>x.id===t.id)?.version??0)+1,updatedAt:stamp}));
