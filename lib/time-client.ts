...
export class TimeClient{
 ...
 // Reserve the version while automatic course work runs. User edits wait for its
 // receipt, so rollover cannot invalidate this client's own newly queued edits.
 private async rollover(){await this.edit(async()=>{await this.loadPending();if(this.pending.length||this.stopped)return;const result=await timeRequest<{version:number;changed:boolean;snapshot?:TimeSnapshot}>('/api/time/courses/rollover',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({space:this.space})});if(result.snapshot)await this.accept(result.snapshot);else if(result.changed)await this.refresh();if(Number.isSafeInteger(result.version))this.acknowledgedVersion=Math.max(this.acknowledgedVersion,result.version);});}
 private async run(){try{
  ...
  await this.refresh();await this.rollover();this.message=this.pending.length?'有修改需要核对，后续提交已保留':'已同步 · 时间版本 '+this.data?.version;return true;
 }catch(e){
 ...
 async resolve(id:string,retry:boolean){if(this.busy)await this.busy;
  await this.edit(async()=>{await this.loadPending();const item=this.pending.find(p=>p.operationId===id);if(!item||!this.data||this.blocked||item.state==='queued')return;
   ...
   if(retry){const replacement:PendingTime={...item,operationId:crypto.randomUUID(),baseVersion:this.data!.version,state:'queued',error:undefined};await localReplace('time/outbox/'+this.scope()+'/'+id,'time/outbox/'+this.scope()+'/'+replacement.operationId,replacement,'time/archive/'+this.scope()+'/'+id,item);}
   else {await localPut('time/archive/'+this.scope()+'/'+id,item);await localRemove('time/outbox/'+this.scope()+'/'+id);}await this.loadPending();this.emit();
  });await this.sync();
 }
