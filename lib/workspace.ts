...
import {validateCategories,type Category,isCategoryId} from './categories.ts';
export type HistoryData={version:1;events:TaskEvent[];coverage:{since:string;incomplete:boolean;unknown:number}};
export type Settings={density:'compact'|'comfortable';categories?:Category[]};
...
export function validateSettings(v:unknown):Settings{
 ensure(v&&typeof v==='object'&&!Array.isArray(v),'备份设置无效');
 const settings=v as Partial<Settings>;
 ensure(settings.density==='compact'||settings.density==='comfortable','备份设置无效');
 let categories:Category[]|undefined;
 try{categories=validateCategories(settings.categories);}catch(error){throw new AppError(error instanceof Error?error.message:'分类目录无效');}
 return categories===undefined?{density:settings.density}:{density:settings.density,categories};
}
export function validateTask(v:unknown):Task{
 ...
 if(present(t as unknown as Record<string,unknown>,'categoryId'))ensure(isCategoryId(t.categoryId),'分类 ID 无效');
...
export function validateImport(v:unknown):Workspace{ensure(v&&typeof v==='object','备份格式无效');const w=v as Workspace;ensure(w.schemaVersion===1,'不支持此备份版本');ensure(w.space==='personal'||w.space==='demo','备份工作区无效');ensure(Array.isArray(w.tasks)&&w.tasks.length<=5000,'备份最多支持 5000 张卡片');const tasks=w.tasks.map(validateTask);ensure(new Set(tasks.map(t=>t.id)).size===tasks.length,'备份中含有重复事项 ID');validateTaskRelations(tasks);const settings=validateSettings(w.settings);return {...w,tasks,settings};}
export function applyOperation(w:Workspace,op:any,now=new Date()):Workspace{
 ...
 }else if(op.kind==='save') {const source=op.task as Task;const sourceRecord=source as unknown as Record<string,unknown>;const draft=validateTask(source);i=next.tasks.findIndex(t=>t.id===draft.id);if(i>=0){const previous=next.tasks[i];ensure(previous.version===draft.version,'事项版本不一致，请重新读取');draft.createdAt=previous.createdAt;draft.completions=previous.completions;draft.deletedAt=previous.deletedAt;if(!present(sourceRecord,'kind')&&previous.kind!==undefined)draft.kind=previous.kind;if(!present(sourceRecord,'projectId')&&previous.projectId!==undefined)draft.projectId=previous.projectId;if(!present(sourceRecord,'queueReturn')&&previous.queueReturn!==undefined)draft.queueReturn=previous.queueReturn;if(!present(sourceRecord,'categoryId')&&previous.categoryId!==undefined){if(previous.flow===draft.flow)draft.categoryId=previous.categoryId;else delete draft.categoryId;}}else{ensure(next.tasks.length<5000,'最多支持 5000 张卡片');ensure(draft.version===0,'新卡片版本应为 0');draft.createdAt=stamp;draft.completions=[];draft.deletedAt=null;}if(draft.categoryId==='')delete draft.categoryId;draft.version++;draft.updatedAt=stamp;i<0?next.tasks.push(draft):next.tasks[i]=draft;
 }else if(op.kind==='settings'){ensure(['compact','comfortable'].includes(op.density),'显示密度无效');next.settings={...next.settings,density:op.density};
