...
type Props={visible:Task[];flow:string;now:Date;filtering:boolean;busy:boolean;ready:boolean;onOpen:(task:Task)=>void;onFlag:(task:Task)=>void;onComplete:(task:Task)=>void;onCreate:(flow:string)=>void;categories?:readonly Category[]};
export function FlowLane({visible,flow,now,filtering,busy,ready,onOpen,onFlag,onComplete,onCreate,categories}:Props){
 ...
 const card=(task:Task)=><TaskCard key={task.id} task={task} categories={categories} now={now} onOpen={onOpen} onFlag={()=>{if(!busy)onFlag(task);}} onComplete={!task.deletedAt&&task.status!=='已结束'?()=>{if(!busy)onComplete(task);}:undefined}/>;
 return <>
