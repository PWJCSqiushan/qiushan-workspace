...
import {Check,CheckCircle2,ChevronUp,Clock3,Flag,Repeat2,Info} from 'lucide-react';
...
import {CategoryBadge,categoryForTask} from '@/components/category-badge';
...
export function TaskCard({task,onOpen,onFlag,onComplete,now,className,categories}:TaskCardProps){
 ...
 const category=categoryForTask(categories,task as Task & {categoryId?:string});
 const longCoordination=coordination.order.length>3;
 const compactStatus=longCoordination||Math.max(priority.length,coordination.label.length)>4;
 ...
 return <article className={['task redesign-task-card','task--tone-'+tone,task.flagged?'flagged':'',completed?'done':'',className||''].filter(Boolean).join(' ')} data-task-id={task.id} data-card-tone={tone} data-priority={priority} data-coord-letter={task.coordLetter||'未定'} data-due-state={due?.state??'none'}>
  ...
  <div className="task-footer">
   {compactStatus?<span className={'task-status task-status-compact'+(task.status==='正在推进'&&!completed?' status-active':'')} title={statusDetail} aria-label={statusDetail}><Info aria-hidden="true"/><span className="status-compact-mark">状态</span></span>:<span className={'task-status'+(task.status==='正在推进'&&!completed?' status-active':'')} title={statusDetail}>{completed?<CheckCircle2 aria-hidden="true"/>:<i className="status-dot" aria-hidden="true"/>}<span>{status}</span></span>}
