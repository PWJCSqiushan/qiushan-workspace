...
import {rolloverAllCourses} from './lib/time-store';
...
export default {
 ...
 async scheduled(_controller:ScheduledController,_environment:Env,ctx:ExecutionContext){ctx.waitUntil(_controller.cron==='0 18 * * *'?Promise.all([backupAll(_environment.DB,_environment.BACKUPS),backupTimeAll(_environment.DB,_environment.BACKUPS)]):rolloverAllCourses(_environment.DB));}
};
